...
export function SimulationSummary({ result }: { result: EnsembleResult }) {
  const fixed = result.runs.filter(
    (run) => run.absorption?.kind === "fixed",
  ).length;
  const lost = result.runs.filter(
    (run) => run.absorption?.kind === "lost",
  ).length;
  ...
  return (
    <div className="result-summary" aria-live="polite">
      <div>
        ...
        <small>截至第 {result.parameters.generations} 代达到 p = 1</small>
      </div>
      <div>
        <span>丢失 · Loss</span>
        ...
        <small>截至第 {result.parameters.generations} 代达到 p = 0</small>
      </div>
      <div>
        <span>未吸收 · Segregating</span>
